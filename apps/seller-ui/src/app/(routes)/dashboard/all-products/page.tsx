...
import {Search, Pencil, Trash, Eye, Plus, BarChart, Star, ChevronRight, Pen, RotateCcw} from "lucide-react"
    ...
    () => [
      {
        accessorKey: "image",
        header: "Image",
        cell: ({row}: any) => <Image src={row.original?.images[0]?.url} alt={row.original?.images[0]?.url} width={200} height={200}  className='w-12 h-12 rounded-md object-cover'/>
      },
      {
        accessorKey: "name",
        header: "product Name",
        cell: ({row}: any) => {
          const truncatedTitle  = row.original.title.length > 25 ? `${row.original.title.substring(0, 25)}...`
          : row.original.title;
    ...
    <div className='w-full min-h-screen p-8'>
      ...
      <div className='flex justify-between items-center'>
        ...
        <Link
          ...
          className='bg-blue-600 hover:bg-blue-700 px-4 py-2 text-white rounded-lg flex items-center font-semibold gap-2'
        >
