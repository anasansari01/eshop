  ...
  return (
    <div className='w-full min-h-screen p-8'>
      <div className='flex justify-between items-center mb-1'>
        <h2 className='font-semibold text-2xl text-white'>Discount Codes</h2>
        <button 
          className='text-white bg-blue-600 hover:bg-blue-700 rounded-lg px-4 py-2 font-semibold gap-2 flex items-center'
      ...
      <div className="mt-8 bg-gray-900 p-6 rounded-lg shadow-lg">
        ...
        {isLoading ? (
          <p className='text-gray-400 text-center'>Loading discounts...</p>
            ...
            <p className='text-gray-400 text-center'>No Discount Codes Available!</p>
          ):(
          <table className='w-full text-white'>
            ...
            <tbody>
              ...
              <tr className='border-b border-gray-800 hover:bg-gray-800 trasition' key={discount?.id}>
                <td className='p-3'>{discount?.public_name}</td>
                <td className='p-3 capitalize'>{discount?.discountType === "percentage" ? "Percentage (%)" : "Flat ($)"}</td>
                <td className='p-3 capitalize'>{discount?.discountType === "percentage" ? `${discount.discountValue}%` : `$${discount.discountValue}`}</td>
                <td className="p-3">{discount.discountCode}</td>
                <td className="p-3">
                  <button className='text-red-400 hover:text-red-300 transition' onClick={()=>handleDeleteClick(discount)}><Trash size={18}/></button>
                </td>
              </tr>
            ))}
            </tbody>
              ...
              <div className='mt-2'>
                ...
                <Controller
                  ...
                  render={({field})=>(
                    <select
                    {...field}
                    className='w-full border outline-none border-gray-700 bg-transparent text-white  px-2 py-2 rounded-lg'
                    >
